    ...
    <div class="col-md-12 mt-3">
        <div class="card card-bordered">
            <div class="card-header color-head">
                <div class="row">
                    <div class="col-6 font-weight-bold">Kabinet - Create</div>
            ...
            <div class="card-body">
                {!! Form::open(['route' => 'kabinet.store', 'method' => 'post']) !!}
                <div class="form-group row">
                    ...
                    <div class="col-6">
                        ...
                        <select class="form-control  @error('roles_id') is-invalid @enderror" data-width="100%" name="roles_id" id="roles_id">
                            ...
                            <option value="{{ $item->id }}" {{ old('roles_id') == $item->id ? 'selected' : null }}>{{ $item->name }}</option>
                            @endforeach
                        </select>
                        @error('roles_id')
                        <code>{{ $message }}</code>
                ...
                <div class="form-group">
                    ...
                    {!! Form::text('nama_kabinet', null, ['placeholder' => 'Name Kabinet', 'class' => 'form-control']) !!}
                    @error('nama_kabinet')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <button type="submit" class="btn  submit btn-primary"><i class="fa fa-plus-circle" aria-hidden="true"></i> Store</button>
            </div>
        </div>
        {!! Form::close() !!}
    </div>
