...
<script>
    ...
    function CreateDepartement() {
        $('#name_departement').removeClass('is-invalid');
        $('#error_name').hide();
        $('#name_departement').val('');
        $('#loading').hide();
        $('#submit').show();
    }

    function StoreDepartement() {
        $.ajaxSetup({
            headers: {
        ...
        var formSet = $("#DepartementForm").serializeArray();
        console.log(formSet);
        var error = 0;
        ...
        if (error == 0) {
            $.ajax({
                ...
                success: function(data) {
                    $('#createDepartementModal').modal('hide');
    ...
    function UpdateDepartement() {
        ...
        var name = $('#NameEditDepartement').val();
        // console.log(formSet);
        $.ajax({
            type: 'PUT',
            url: "{{ route('departement_update') }}",
            data: {
                id: id,
                name: name
            },
    ...
    function DeleteDepartement(id, name) {
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            }
